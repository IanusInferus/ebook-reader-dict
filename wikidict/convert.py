...
def main(locale: str) -> int:
    ...
    for include_etymology in [True]: # [False, True]:
        ...
        try:
            ...
            log.exception("Error with the StarDict conversion: %s", ex)
        try:
            run_mobi_formatter(output_dir, input_file, locale, normal_words, {}, include_etymology=include_etymology)
        except Exception as ex:
        ...
        if len(supplementary_words) > 0:
            ...
            try:
                run_mobi_formatter(output2_dir, input_file, locale, supplementary_words, {}, include_etymology=include_etymology)
            except Exception as ex:
                log.exception("Error with the Mobi conversion: %s", ex)
